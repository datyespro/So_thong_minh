-- TIP-PAY-DATE (VĐ1): commit_payment v2 — ghi paid_at theo ngày người dùng nói.
--
-- Forward-only. Đổi chữ ký (thêm p_business_date) => DROP bản 5-tham-số cũ trước
-- khi CREATE để KHÔNG tạo 2 overload cùng tên. Mọi thứ khác giữ nguyên so với
-- 20260531030000_tip007b: idempotency, guard overpay (VĐ3 xử riêng — KHÔNG đụng),
-- sync_customer_debt_total, audit_log. created_at vẫn now() (audit thật không đổi).
--
-- paid_at lưu VN-midnight của ngày đã nói (hiển thị /customers/[id] theo APP_TIME_ZONE
-- ra đúng ngày). business_date trả về tính theo VN wall-clock để KHÔNG phụ thuộc
-- session timezone của Postgres (paid_at::date thô sẽ lệch -1 ngày khi session=UTC).

DROP FUNCTION IF EXISTS public.commit_payment(TEXT, UUID, NUMERIC, TEXT, TEXT);
...
BEGIN
  ...
  -- Plan B: đọc nợ hiện tại và BLOCK trả quá nợ. (GIỮ NGUYÊN — VĐ3 xử riêng.)
